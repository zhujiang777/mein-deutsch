...
## 0. 旧客户端 Gist 同步会覆盖生词本（低风险）

生词本从 `md.wordbook.v1` 起纳入导出和 Gist 同步。仍在运行旧版客户端的用户若把旧载荷推送到 Gist，整包“新者胜”合并会不包含该字段，从而清空新客户端的生词本。当前为个人单用户 PWA，窗口很小；在所有设备刷新到含生词本的版本前，避免从旧标签页手动同步。
